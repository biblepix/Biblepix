...
#pack $w.topF $w.botF
#Create scrollbars - TODO Delete or Fix (move content of canvas not canvas itself)
...
#TODO not working
$resizepic::c bind {mv,img} <Key-uparrow> {%W yview scroll 1 "units"}
...
puts "orig: $origW $origH"
  ...
  image create photo reposCanvPic
	...
	set scaleFactor 2
	reposCanvPic copy $pic -subsample $scaleFactor
  reposCanvPic conf -width [expr $origW / $scaleFactor] -height [expr $origH / $scaleFactor]
...
puts "win: $winX $winY"
  ...
  set repospic::canv [canvas $repospic::w.reposCanv -bg lightblue -height $canvY -width $canvX]
