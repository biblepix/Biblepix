...
#called by setFlags in SetupBuildGui
 namespace eval flag {
    ...
    proc circle color {
        ...
        set xm [expr ($x+$w)/2]
        set ym [expr ($y+$h)/2+2]
        $c create oval [expr $xm-$r] [expr $ym-$r] [expr $xm+$r] [expr $ym+$r] -fill $color -outline $color
    }
    proc hori args {
        ...
        set y0 [expr $y+2+$b]
        foreach i $args {
            $c create rect $x $y0 [expr $x+$w+2*$b] [expr round($y0+$dy)] -fill $i -outline $i
            set y0 [expr $y0+$dy]
        }
    }
    proc vert args {
        ...
        set x0 [expr $x+2]
        foreach i $args {
            $c create rect $x0 $y [expr round($x0+$dx)] [expr $y+$w] -fill $i -outline $i
            set x0 [expr $x0+$dx]
        }
    }
    proc cross args {
        ...
        set y2 [expr $h*0.6+1]
        foreach i $args {
            $c create rect $x $y1 [expr $x+$w+2*$b] $y2 -fill $i -outline $i
            $c create rect $x1 $y $x2 [expr $y+$h+2] -fill $i -outline $i
            set x1 [expr $x1*1.1]
            set x2 [expr $x2/1.1]
            set y1 [expr $y1*1.1]
            set y2 [expr $y2/1.1]
    ...
    proc ncross args {
        ...
        foreach i $args {
            $c create rect $x $y1 [expr $x+$w+2*$b] $y2 -fill $i -outline $i
            $c create rect $y1 $y $y2 [expr $y+$h+1] -fill $i -outline $i
            set y1 [expr $y1*1.1]
            set y2 [expr $y2/1.1]
        }
    }
    proc scross color {
        ...
        set y2 [expr $y+$h*0.6]
        set y3 [expr $y+$h*0.8]
        $c create rect $x0 $y1 $x3 $y2 -fill $color -outline $color
        $c create rect $x1 $y0 $x2 $y3 -fill $color -outline $color
    }
    proc x args {
        upvar c c x x y y w w h h
        set width [expr round($w/10.)]
        foreach i $args {
            $c create line $x [expr $y+2] [expr $x+$w] [expr $y+$h] -fill $i -width $width
            $c create line $x [expr $y+$h] [expr $x+$w] [expr $y+2] -fill $i -width $width
    ...
    proc stripes {n c0 c1} {
        ...
        for {set i 0} {$i<$n} {incr i} {
            ...
            $c create rect $x $y0 [expr $x+$w+2*$b] [expr round($y0+$dy)] -fill $color -outline $color
            set y0 [expr $y0+$dy]
        }
    }
    proc sun color {
        ...
        $c create oval [expr $x0-$r/2.] [expr $y0-$r/2.] [expr $x0+$r/2.] [expr $y0+$r/2.] -fill $color
    ...
    proc triangle color {
        ...
        $c create poly $x $y $x1 [expr ($y+$h)/2.+2] $x [expr $y+$h+2] -fill $color
    }
    ...
    proc moon color {
        ...
        $c create oval [expr $x0-$r] [expr $y0-$r] [expr $x0+$r] [expr $y0+$r] -fill $color -outline $color
