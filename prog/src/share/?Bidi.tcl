...
#    set newtext [string map { \{ {} \} {} \\ {} } $newtext]
    #remove any unwanted {curly breaks}
    set newtext [string map { \u23A8 {} \u23AC {}  } $s]
    return $newtext
...
puts $htmcode
      ...
      if {$pos == $firstLetterPos} {
        set utfchar [formatArLetter $htmcode i $prevLinking]
#puts "First $pos"
      ##set final letter form  
      } elseif {$pos == $lastLetterPos} { 
#puts "Last $pos"
        set utfchar [formatArLetter $htmcode f $prevLinking]

      ##set middle letter form    
      } else {
        set utfchar [formatArLetter $htmcode m $prevLinking]
      }
  ...
  proc formatArLetter {htmcode form prevLinking} {
    global [namespace current]::$htmcode
    upvar [namespace current]::$htmcode letterArr
